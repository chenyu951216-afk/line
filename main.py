...
async def line_push_forward(to: str, text: str, image_urls: list[str]) -> None:
    ...
    if text_messages:
        await line_api_post("message/push", {"to": to, "messages": text_messages[:5]})
    images = line_image_messages(image_urls)
    for start in range(0, len(images), 5):
        await line_api_post("message/push", {"to": to, "messages": images[start:start + 5]})
...
async def process_message(
    ...
    try:
        ...
        await db.log("INFO", "line", "Message forwarded to LINE", {"chat_id": chat_id, "message_id": message_key, "recipient_count": len(recipients), "image_count": len(line_image_url_list)})

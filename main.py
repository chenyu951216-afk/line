...
class RuntimeSettings(BaseModel):
    ...
    discord_default_mention: str = Field(default_factory=lambda: env_str("DISCORD_DEFAULT_MENTION") or env_str("DISCORD_NOTIFY_MENTION") or env_str("DISCORD_NOTIFY_USER_ID"))
    discord_dm_user_id: str = Field(default_factory=lambda: env_str("DISCORD_DM_USER_ID") or env_str("DISCORD_NOTIFY_DM_USER_ID") or env_str("DISCORD_NOTIFY_USER_ID"))
    discord_dm_always_send: bool = Field(default_factory=lambda: env_bool("DISCORD_DM_ALWAYS_SEND", True))
...
class Database:
    ...
    async def get_settings(self) -> RuntimeSettings:
        ...
        if os.getenv("DISCORD_DEFAULT_MENTION") not in (None, "") or os.getenv("DISCORD_NOTIFY_MENTION") not in (None, "") or os.getenv("DISCORD_NOTIFY_USER_ID") not in (None, ""):
            base["discord_default_mention"] = env_str("DISCORD_DEFAULT_MENTION") or env_str("DISCORD_NOTIFY_MENTION") or env_str("DISCORD_NOTIFY_USER_ID")
        if os.getenv("DISCORD_DM_USER_ID") not in (None, "") or os.getenv("DISCORD_NOTIFY_DM_USER_ID") not in (None, "") or os.getenv("DISCORD_NOTIFY_USER_ID") not in (None, ""):
            base["discord_dm_user_id"] = env_str("DISCORD_DM_USER_ID") or env_str("DISCORD_NOTIFY_DM_USER_ID") or env_str("DISCORD_NOTIFY_USER_ID")
        if os.getenv("DISCORD_DM_ALWAYS_SEND") not in (None, ""):
            base["discord_dm_always_send"] = env_bool("DISCORD_DM_ALWAYS_SEND", bool(base.get("discord_dm_always_send")))
...
def discord_bot_token() -> str:
    return env_str("DISCORD_BOT_TOKEN") or env_str("DISCORD_DM_BOT_TOKEN") or env_str("DISCORD_NOTIFY_BOT_TOKEN")


def discord_bot_auth_header() -> str:
...
def discord_dm_user_id(settings: RuntimeSettings) -> str:
    config = discord_mention_config(settings.discord_dm_user_id)
    return config["users"][0] if config["users"] else ""
...
def available_destinations(settings: RuntimeSettings) -> list[dict[str, Any]]:
    ...
    dm_user_id = discord_dm_user_id(settings)
    if dm_user_id:
        destinations.append(
            {
                "id": "discord-dm-default",
                "type": "discord_dm",
                "name": "Discord DM",
                "enabled": bool(discord_bot_token()),
                "target": mask_secret(dm_user_id, 4),
                "user_id": dm_user_id,
                "reason": "" if discord_bot_token() else "DISCORD_BOT_TOKEN missing",
            }
        )
...
def selected_destinations_for_source(settings: RuntimeSettings, chat_id: str) -> list[dict[str, Any]]:
    ...
    if route:
        ...
        chosen = [dest for dest in destinations if dest["id"] in selected and dest.get("enabled", True)]
        if selected and not chosen:
            chosen = [dest for dest in destinations if dest.get("enabled", True)]
    else:
        chosen = [dest for dest in destinations if dest.get("enabled", True)]
    if settings.discord_dm_always_send:
        chosen_ids = {str(dest.get("id") or "") for dest in chosen}
        for dest in destinations:
            if str(dest.get("id") or "") == "discord-dm-default" and dest.get("enabled", True) and "discord-dm-default" not in chosen_ids:
                chosen.append(dest)
                break
    return chosen
...
def route_mention(route: dict[str, Any], destination: dict[str, Any], default_mention: str = "") -> str:
    return str(route.get("mention") or destination.get("mention") or default_mention or "").strip()
...
def discord_mention_hint_text(raw_mention: str) -> str:
    config = discord_mention_config(raw_mention)
    if config["everyone"] and not config["users"] and not config["roles"]:
        return "手機沒跳時，請改用 <@你的使用者ID>；@everyone/@here 很常被 Discord 設定壓掉。"
    if config["roles"] and not config["users"]:
        return "role 通知仍受 Discord 身分組可提及、頻道與手機端設定影響；最穩是加上 <@你的使用者ID>。"
    if config["users"]:
        return "使用者 mention 已設定，通常是最穩的手機推播方式。"
    if config["invalid"]:
        return "請填 <@使用者ID>、<@&身分組ID>、user:ID、role:ID、@everyone 或 @here。"
    return "要穩定跳手機通知，建議填 <@你的使用者ID>。"
...
async def discord_bot_api_post(path: str, payload: dict[str, Any]) -> dict[str, Any]:
    auth = discord_bot_auth_header()
    if not auth:
        raise RuntimeError("DISCORD_BOT_TOKEN / DISCORD_DM_BOT_TOKEN is not configured")
    async with httpx.AsyncClient(timeout=20) as client:
        response = await client.post(
            f"https://discord.com/api/v10/{path.lstrip('/')}",
            headers={"Authorization": auth, "Content-Type": "application/json"},
            json=payload,
        )
    if response.status_code >= 300:
        raise RuntimeError(f"Discord Bot API {response.status_code}: {response.text}")
    try:
        return response.json() if response.text else {"ok": True}
    except Exception:
        return {"ok": True, "text": response.text}
...
async def discord_dm_post(user_id: str, payload: dict[str, Any]) -> dict[str, Any]:
    user_id = str(user_id or "").strip()
    if not user_id:
        raise RuntimeError("Discord DM user id is not configured")
    dm_channel = await discord_bot_api_post("users/@me/channels", {"recipient_id": user_id})
    channel_id = str(dm_channel.get("id") or "").strip()
    if not channel_id:
        raise RuntimeError(f"Discord DM channel creation failed: {dm_channel}")
    message_payload = {
        "content": truncate_text(str(payload.get("content") or ""), 1900),
        "embeds": payload.get("embeds") or [],
        "allowed_mentions": payload.get("allowed_mentions") or {"parse": []},
    }
    return await discord_bot_api_post(f"channels/{channel_id}/messages", message_payload)
...
def build_discord_payload(
    ...
    raw_mention = route_mention(route, destination, default_mention)
...
async def process_message(
    ...
    try:
        ...
        if route:
            selected_route_destinations = {str(item) for item in route.get("destinations", []) if str(item).strip()}
            chosen_destination_ids = {str(item.get("id") or "") for item in destinations}
            if selected_route_destinations and not (selected_route_destinations & chosen_destination_ids):
                await db.log(
                    "WARN",
                    "router",
                    "Source route destinations were stale or disabled; falling back to enabled destinations",
                    {
                        "chat_id": chat_id,
                        "message_id": message_key,
                        "selected_destinations": sorted(selected_route_destinations),
                        "fallback_destinations": sorted(chosen_destination_ids),
                    },
                )
        if not destinations:
        ...
        for destination in destinations:
            ...
            try:
                if dest_type == "line":
                    ...
                    mention_config = discord_mention_config(route_mention(route, destination, settings.discord_default_mention))
                    mention_summary = {
                ...
                elif dest_type == "discord_dm":
                    user_id = str(destination.get("user_id") or discord_dm_user_id(settings)).strip()
                    if not discord_bot_token():
                        delivery_results.append({"destination": dest_id, "status": "skipped", "reason": "DISCORD_BOT_TOKEN missing"})
                        continue
                    if not user_id:
                        delivery_results.append({"destination": dest_id, "status": "skipped", "reason": "Discord DM user id missing"})
                        continue
                    payload = build_discord_payload(
                        destination,
                        {**route, "mention": ""},
                    ...
                    await discord_dm_post(user_id, payload)
                    delivery_results.append({"destination": dest_id, "status": "sent", "type": "discord_dm", "user_id": mask_secret(user_id, 4)})
...
def telegram_media_class_name(value: Any) -> str:
    return value.__class__.__name__ if value is not None else ""


def telegram_media_sequence(value: Any) -> list[Any]:
    if value is None:
        return []
...
def telegram_document_mime(message: Any) -> str:
    document = getattr(message, "document", None)
    media = getattr(message, "media", None)
    if document is None and media is not None:
        document = getattr(media, "document", None)
    return str(getattr(document, "mime_type", "") or "").lower()


def telegram_file_ext(message: Any) -> str:
    ext = str(getattr(getattr(message, "file", None), "ext", "") or "").lower()
    if ext.startswith(".") and len(ext) <= 8:
        return ext
    mime = telegram_document_mime(message)
    if mime == "image/png":
        return ".png"
    if mime == "image/webp":
        return ".webp"
...
def telegram_paid_media_info(message: Any) -> dict[str, Any]:
    media = getattr(message, "media", None)
    media_name = telegram_media_class_name(media)
    extended_media = telegram_media_sequence(getattr(media, "extended_media", None))
    paid = "PaidMedia" in media_name or bool(extended_media) or getattr(media, "stars_amount", None) not in (None, "")
    return {
        "paid": bool(paid),
        "stars": getattr(media, "stars_amount", None),
        "count": len(extended_media),
        "extended_media": extended_media,
        "class_name": media_name,
    }


def telegram_media_has_spoiler(message: Any) -> bool:
    media = getattr(message, "media", None)
    if bool(getattr(message, "has_spoiler", False) or getattr(message, "spoiler", False)):
        return True
    if media is not None and bool(getattr(media, "spoiler", False) or getattr(media, "has_spoiler", False)):
        return True
    document = getattr(message, "document", None)
    return bool(getattr(document, "spoiler", False) or getattr(document, "has_spoiler", False))


def telegram_message_has_image_like_media(message: Any) -> bool:
    if getattr(message, "photo", None):
        return True
    if telegram_document_mime(message).startswith("image/"):
        return True
    media = getattr(message, "media", None)
    media_name = telegram_media_class_name(media)
    if "Photo" in media_name or "PaidMedia" in media_name:
        return True
    paid_info = telegram_paid_media_info(message)
    for item in paid_info["extended_media"]:
        item_name = telegram_media_class_name(item)
        inner = getattr(item, "media", None)
        inner_name = telegram_media_class_name(inner)
        if "Photo" in item_name or "Photo" in inner_name or getattr(item, "thumb", None):
            return True
    return False


def telegram_media_notes(message: Any, downloaded: bool, download_error: str = "") -> list[str]:
    notes: list[str] = []
    paid_info = telegram_paid_media_info(message)
    if paid_info["paid"]:
        detail = []
        if paid_info["stars"] not in (None, ""):
            detail.append(f"{paid_info['stars']} Stars")
        if paid_info["count"]:
            detail.append(f"{paid_info['count']} 個媒體")
        suffix = "，".join(detail)
        if suffix:
            suffix = f"（{suffix}）"
        if downloaded:
            notes.append(f"Telegram 付費媒體{suffix}：目前帳號可取得，已嘗試轉出圖片。")
        else:
            notes.append(f"Telegram 付費媒體{suffix}：未解鎖、權限不足或 Telegram 未提供原圖，無法繞過 Stars 取得清晰照片。")
    if telegram_media_has_spoiler(message):
        notes.append("Telegram 打霧/spoiler 媒體：已嘗試下載原圖。" if downloaded else "Telegram 打霧/spoiler 媒體：偵測到媒體，但下載失敗，已先轉出提示。")
    if download_error:
        notes.append(f"Telegram 圖片下載失敗：{download_error}")
    return notes


async def download_telegram_image_like_media(message: Any, client: Any = None) -> tuple[Optional[str], str]:
    suffix = telegram_file_ext(message)
    candidates: list[tuple[str, Any]] = [("message", message)]
    paid_info = telegram_paid_media_info(message)
    for index, item in enumerate(paid_info["extended_media"], start=1):
        candidates.append((f"paid_media_{index}", item))
        inner = getattr(item, "media", None)
        if inner is not None:
            candidates.append((f"paid_media_{index}_inner", inner))

    last_error = ""
    for label, candidate in candidates:
        fd, tmp_path = tempfile.mkstemp(prefix="tg_signal_", suffix=suffix)
        os.close(fd)
        try:
            if label == "message" and hasattr(candidate, "download_media"):
                result = await candidate.download_media(file=tmp_path)
            elif client is not None and hasattr(client, "download_media"):
                result = await client.download_media(candidate, file=tmp_path)
            else:
                result = None
            downloaded_path = str(result or tmp_path)
            if os.path.exists(downloaded_path) and os.path.getsize(downloaded_path) > 0:
                if downloaded_path != tmp_path and os.path.exists(tmp_path):
                    try:
                        os.remove(tmp_path)
                    except OSError:
                        pass
                return downloaded_path, ""
            last_error = f"{label}: no downloadable file"
        except Exception as exc:
            last_error = f"{label}: {redact_secrets(str(exc))}"
...
async def handle_incoming_message(event: events.NewMessage.Event) -> None:
    ...
    try:
        ...
        has_image = telegram_message_has_image_like_media(event.message)
        if has_image and settings.allow_image_signal:
            tmp_image, download_error = await download_telegram_image_like_media(event.message, getattr(event, "client", None) or telegram_client)
            media_notes.extend(telegram_media_notes(event.message, bool(tmp_image), download_error))
            if download_error:
                await db.log(
                    "WARN",
                    "telegram",
                    "Telegram media detected but download failed; forwarding text fallback",
                    {
                        "chat_id": source_chat_id,
                        "message_id": message_id,
                        "media_type": telegram_media_class_name(getattr(event.message, "media", None)),
                        "error": download_error,
                    },
                )
        elif has_image:
            media_notes.extend(telegram_media_notes(event.message, False, "ALLOW_IMAGE_SIGNAL is disabled"))
        else:
            media_notes.extend(telegram_media_notes(event.message, False, ""))
        if media_notes:
            note_text = "\n".join(f"[{note}]" for note in media_notes)
            text = (text + "\n" + note_text).strip() if text else note_text
        ...
        if not saved:
            ...
            return
        await db.log("INFO", "telegram", "Telegram message accepted", {"chat_id": source_chat_id, "message_id": message_id, "has_image": has_image, "media_type": telegram_media_class_name(getattr(event.message, "media", None)), "media_note_count": len(media_notes)})
...
async def handle_discord_message(message: Any, force_update: bool = False) -> None:
    ...
    try:
        ...
        if not saved:
            await db.log("INFO", "discord", "Duplicate Discord message ignored", {"chat_id": chat_id, "message_id": message_id, "force_update": force_update})
            return
        ...
        await db.log(
            "ERROR",
            "discord",
            "Incoming Discord message handling failed",
            {
                "chat_id": chat_id,
                "message_id": message_id,
                "error": redact_secrets(str(exc)),
...
async def process_discord_message(
    ...
    try:
        ...
        await db.log(
            "ERROR",
            "discord",
            "Discord source message forwarding failed",
            {
                "chat_id": chat_id,
                "message_id": message_id,
                "error": redact_secrets(str(exc)),
...
async def routes_page() -> HTMLResponse:
    ...
    <section class="card">
      ...
      <form method="post" action="/routes/destinations">
        ...
        <label>重要通知 mention（建議填 &lt;@你的使用者ID&gt;，比 @everyone 更穩）</label><input name="mention" placeholder="<@123456789012345678>">
    ...
    for dest in destinations:
        ...
        action_parts = []
        if str(dest.get("type")) in {"discord_webhook", "discord_dm"}:
            action_parts.append(
        ...
        action = f"<div class='action-stack'>{''.join(action_parts)}</div>"
        mention_cell = "-"
        if str(dest.get("type")) == "discord_webhook":
            raw_mention = str(dest.get("mention") or "")
            effective_mention = raw_mention or settings.discord_default_mention
            mention_status = discord_mention_status_text(effective_mention)
            mention_hint = discord_mention_hint_text(effective_mention)
            default_note = "（套用全域預設）" if not raw_mention and settings.discord_default_mention else ""
            mention_cell = f"<div>{html.escape(mention_status + default_note)}</div><div class='muted'>{html.escape(mention_hint)}</div>"
            if str(dest.get("id", "")).startswith("discord-env-"):
                mention_cell += "<div class='muted'>env 目標請改 Zeabur 的 DISCORD_WEBHOOKS 第三段。</div>"
            else:
                mention_cell += (
                    "<form method='post' action='/routes/destinations/update'>"
                    f"<input type='hidden' name='destination_id' value='{html.escape(dest_id)}'>"
                    f"<input name='mention' value='{html.escape(raw_mention)}' placeholder='<@你的使用者ID>'>"
                    "<button class='btn' type='submit'>更新 mention</button>"
                    "</form>"
                )
        elif str(dest.get("type")) == "discord_dm":
            auto_text = "一律隨每則訊息私訊推送" if settings.discord_dm_always_send else "依來源路由勾選推送"
            mention_cell = f"<div>Discord 私訊</div><div class='muted'>{html.escape(auto_text)}</div>"
        body += (
            "<tr>"
    ...
    for idx, source in enumerate(source_options):
        ...
        body += (
            ...
            f"<td><input name='mention_{idx}' value='{html.escape(str(route.get('mention') or ''))}' placeholder='建議 <@你的使用者ID>'></td>"
            "</tr>"
        )
...
async def test_route_destination(destination_id: str = Form(...)) -> RedirectResponse:
    ...
    if not destination or str(destination.get("type")) not in {"discord_webhook", "discord_dm"}:
        await db.log("WARN", "ui", "Discord test notification destination not found", destination_id)
        return redirect("/routes")

    mention_config = discord_mention_config(route_mention({}, destination, settings.discord_default_mention))
    payload = build_discord_payload(
        destination,
    ...
    try:
        if str(destination.get("type")) == "discord_dm":
            user_id = str(destination.get("user_id") or discord_dm_user_id(settings)).strip()
            await discord_dm_post(user_id, payload)
        else:
            await discord_webhook_post(str(destination.get("url") or ""), payload)
...
@app.post("/routes/destinations/update", dependencies=[Depends(require_auth)])
async def update_route_destination(destination_id: str = Form(...), mention: str = Form("")) -> RedirectResponse:
    settings = await db.get_settings()
    destinations = [item for item in settings.notification_destinations if isinstance(item, dict)]
    updated = False
    for item in destinations:
        if str(item.get("id") or "") == str(destination_id) and str(item.get("type") or "") == "discord_webhook":
            item["mention"] = mention.strip()
            updated = True
            break
    if updated:
        await db.set_setting("notification_destinations", destinations)
        config = discord_mention_config(mention)
        await db.log(
            "INFO",
            "ui",
            "Discord notification destination mention updated",
            {
                "destination": destination_id,
                "mention": {
                    "enabled": bool(config["valid"]),
                    "users": len(config["users"]),
                    "roles": len(config["roles"]),
                    "everyone": bool(config["everyone"]),
                    "invalid": bool(config["invalid"]),
                },
            },
        )
    else:
        await db.log("WARN", "ui", "Discord notification destination mention update skipped", destination_id)
    return redirect("/routes")
...
async def settings_page() -> HTMLResponse:
    ...
    <section class="card">
      <form method="post" action="/settings">
        ...
        <label>Discord 預設手機通知 mention</label>
        <input name="discord_default_mention" value="{html.escape(settings.discord_default_mention)}" placeholder="<@你的使用者ID> 或直接填 Discord 使用者ID">
        <p class="muted">來源與通知目標都沒填 mention 時，每則 Discord webhook 會自動套用這裡；要穩定跳手機通知，建議填自己的 Discord 使用者 ID。</p>
        <label>Discord DM 使用者 ID</label>
        <input name="discord_dm_user_id" value="{html.escape(settings.discord_dm_user_id)}" placeholder="你的 Discord 使用者 ID">
        <label><input type="checkbox" name="discord_dm_always_send" value="true" {checked(settings.discord_dm_always_send)}> Discord DM 一律隨每則訊息推送</label>
        <p class="muted">需要在 Zeabur 設定 DISCORD_BOT_TOKEN，並把 bot 加到你所在的 Discord 伺服器；私訊通知通常比頻道 mention 更容易跳手機推播。</p>
    ...
    <section class="card">
      ...
      <table>
        ...
        <tr><td>DISCORD_BOT_TOKEN</td><td>{zh_bool(bool(discord_bot_token()))}</td></tr>
        <tr><td>Discord 預設手機通知 mention</td><td>{html.escape(discord_mention_status_text(settings.discord_default_mention))}</td></tr>
        <tr><td>Discord DM 使用者 ID</td><td>{html.escape(discord_dm_user_id(settings) or "未設定 / 格式無效")}</td></tr>
        <tr><td>Discord DM 一律推送</td><td>{zh_bool(settings.discord_dm_always_send)}</td></tr>
...
async def save_settings(
    ...
    discord_default_mention: str = Form(""),
    discord_dm_user_id: str = Form(""),
    discord_dm_always_send: Optional[str] = Form(None),
    ...
    updates = {
        ...
        "discord_default_mention": discord_default_mention.strip(),
        "discord_dm_user_id": discord_dm_user_id.strip(),
        "discord_dm_always_send": discord_dm_always_send == "true",

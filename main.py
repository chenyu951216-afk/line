...
def build_discord_payload(
    ...
    mention = route_mention(route, destination)
    ...
    content = (mention + "\n" if mention else "") + truncate_text(headline, 180)
        ...
        for destination in destinations:
            ...
            try:
                if dest_type == "line":
                    ...
                    delivery_results.append({"destination": dest_id, "status": "sent", "type": "discord_webhook"})
...
async def routes_page() -> HTMLResponse:
    ...
    """
    body += "<section class='card'><h2>通知目標</h2><div class='table-wrap'><table><tr><th>名稱</th><th>類型</th><th>目標</th><th>狀態</th><th>操作</th></tr>"
    for dest in destinations:
        action = ""
        if str(dest.get("id")) != "line-default" and not str(dest.get("id", "")).startswith("discord-env-"):
            action = f"<form method='post' action='/routes/destinations/delete'><input type='hidden' name='destination_id' value='{html.escape(str(dest.get('id')))}'><button class='btn danger' type='submit'>刪除</button></form>"
        elif str(dest.get("id", "")).startswith("discord-env-"):
            action = "<span class='muted'>env</span>"
        body += (
            "<tr>"
